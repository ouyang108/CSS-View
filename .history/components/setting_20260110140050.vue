<script setup lang="ts">
// 实时预览数据
const previewData = ref({
  borderColor: '#000000',
  borderStyle: 'solid',
})
function updatePreview(e: Event) {
  const inputEvent = e as InputEvent

  // 2. 对 target 进行类型断言 + 存在性检查（双重安全）
  const target = inputEvent.target
  if (!(target instanceof HTMLInputElement)) {
    // 非输入框元素触发时，直接返回，避免报错
    console.warn('触发事件的元素不是输入框')
    return
  }
  const inputValue = target.value
  previewData.value.borderColor = inputValue
}
</script>

<template>
  <div>
    <div class="header">
      <h1>css Inspect配置</h1>
    </div>

    <div class="config-section">
      <!-- 边框颜色配置 -->
      <div class="config-item">
        <label class="config-label">边框颜色</label>
        <input id="borderColor" v-model="previewData.borderColor" type="color" value="#000000">
      </div>

      <!-- 边框样式配置 -->
      <div class="config-item">
        <label class="config-label">边框样式</label>
        <select id="borderStyle" v-model="previewData.borderStyle">
          <option value="solid">
            实线
          </option>
          <option value="dashed">
            虚线
          </option>
          <option value="dotted">
            点线
          </option>
          <option value="double">
            双线
          </option>
        </select>
      </div>

      <!-- 按键配置开关 -->
      <div class="config-item">
        <label class="config-label">启用按键触发</label>
        <label class="toggle-switch">
          <input id="enableKeyConfig" type="checkbox">
          <span class="slider" />
        </label>
      </div>

      <!-- 自定义按键配置 -->
      <div id="keyConfigContainer" class="config-item">
        <label class="config-label">触发按键</label>
        <p class="config-desc">
          输入自定义按键（例如：Ctrl+Shift+L）
        </p>
        <input id="customKey" type="text" placeholder="请输入触发按键" disabled>
      </div>
    </div>

    <!-- 预览区域 -->
    <div class="preview-section">
      <p class="preview-text">
        实时预览
      </p>
      <div id="previewBox" class="preview-box" :style="previewData">
        预览区域
      </div>
    </div>

    <!-- 操作按钮 -->
    <div class="btn-container">
      <button id="saveBtn" class="btn-save">
        保存配置
      </button>
      <button id="resetBtn" class="btn-reset">
        重置默认
      </button>
    </div>
  </div>
</template>

<style  scoped>
    @import './setting.css';
</style>
