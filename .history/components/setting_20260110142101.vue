...
const { state: configData } = useReset({
  switch: false,
  keyConfig: '',
})
function resets() {
...
// 保存配置
function saveConfig() {
  storage.setItem('cssInspectConfig', JSON.stringify(configData))
}
watch(() => configData.switch, (newVal) => {
  if (!newVal) {
...
<template>
  <div>
    ...
    <div class="config-section">
      ...
      <div class="config-item">
        ...
        <label class="toggle-switch">
          <input id="enableKeyConfig" v-model="configData.switch" type="checkbox">
          <span class="slider" />
        </label>
      </div>
      ...
      <div id="keyConfigContainer" class="config-item">
        ...
        <input id="customKey" v-model="configData.keyConfig" type="text" placeholder="请输入触发按键" :disabled="!configData.switch">
      </div>
    </div>
    ...
    <div class="preview-section">
      ...
      <div id="previewBox" class="preview-box" :style="previewData">
        预览区域
      </div>
    </div>
