...
// 监听配置变化的函数
function handleConfigChange(changes: Record<string, any>, areaName: string) {
  if (areaName === 'local' && changes[local_CONFIG]) {
    const newConfig = changes[local_CONFIG].newValue
    if (newConfig) {
      highlightLayerStyle.value = {
        ...highlightLayerStyle.value,
        ...newConfig,
